...
import jojoaky.substance.util.ItemContainer;
...
public class PipeItem extends ConsumableItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!container.isEmpty() && !player.isCrouching()) {
    ...
    protected void onStopConsuming(ItemStack stack, Level level, LivingEntity entity, int useDuration) {
        ...
        SubstanceEffectHelper.applyStackingEffect(entity, ModEffects.HAZE.get(), useDuration * 5, 600, 1);

        var processedItems = new HashSet<Item>();

        container.getItems().forEach(
                item -> handleConsumeIngredient(item, processedItems, stack, level, entity, useDuration)
        );
    ...
    public static void dropContents(ItemStack pipeStack, Level level, Vec3 pos) {
        ...
        for (ItemStack content : container.getItems()) {
            if (!content.isEmpty()) {
