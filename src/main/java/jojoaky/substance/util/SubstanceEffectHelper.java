...
public class SubstanceEffectHelper {
    ...
    public static void applyStackingEffect(LivingEntity entity, MobEffect effect, int duration, int durationPerAmplifier, int maxAmplifier) {
        ...
        var effectInstance = entity.getEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect));
        ...
        entity.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect), newDuration, newAmplifier));
    }

    public static void applyEffectBase(LivingEntity entity, MobEffect effect, int duration, int amplifier) {
        if (entity.level().isClientSide) return;

        entity.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect), duration, amplifier));
    }
}
